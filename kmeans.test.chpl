...
(0.0122162, 0.309424, 0.420688));
  ...
  writeln("points");
  writeln(points);
  writeln("points locales");
  for (i,j) in PointsSpace {
    if (j%numDim == 0) {
       writeln();
    }
    write(points[i,j].locale, " ");
  }
  writeln();
  ...
  while (step < numIter && !converged) {
    ...
    writeln("step ", step, " localCurrCenters");
    ...
    forall loc in Locales {
      on loc do {
        cobegin {
	  for (i,j) in ClusterSpace { // was forall
	    tmpLCU[i,j].add(localCenterUpdates[i,j].read());
	  }
	  for i in ClusterNumSpace { // was forall
